...
ind_means_mat = matrix(NA, 200, (3*nSub))
...
for (x in 1:200){
  ...
  gamma_ind <- par[6 + 1:(nSub-1)]
  alpha_ind <- par[6 + (nSub-1) + 1:(nSub-1)]
  theta_ind <- par[6 + 2*(nSub-1) + 1:(nSub-1)]
  
  gamma_ind <- c(gamma_ind, nSub*gamma_group_mean - sum(gamma_ind))
  alpha_ind <- c(alpha_ind, nSub*alpha_group_mean - sum(alpha_ind))
  theta_ind <- c(theta_ind, nSub*theta_group_mean - sum(theta_ind))
  ...
  #MEANS (individual level)
  ind_means_mat[x,] = c(gamma_ind, alpha_ind, theta_ind)  
...
write.csv(ind_means_mat, file = "MeansMat.csv")
